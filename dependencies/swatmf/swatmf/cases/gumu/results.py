...
from swatmf import analyzer, gumu_pst_utils, handler, hg, swatmf_pst_utils
...
def temp_plot(
    stf_obd_df, obd_col, wb_df, viz_ts, gw_df, grid_id, gw_obd_df, gw_obd_col
):
    """_summary_
    ...
    subplots = fig.subfigures(
        4,
        1,
        height_ratios=[0.2, 0.2, 0.2, 0.4],
        #   hspace=-0.05
    )

    ax0 = subplots[0].subplots(1, 1)
    ax1 = subplots[1].subplots(
        1,
        2,
        sharey=False,
        gridspec_kw={
...
def hg_yield02():
    # yield
    hg_yld = mod1.hg_yield(1)[cal_start:cal_end] * 0.001  # mg to g
    hg_yldm = hg_yld.resample("ME").sum()
    hg_yldm = hg_yldm.groupby(hg_yldm.index.month).mean()
    hg_yldm['Dissolved'] = hg_yldm[["Hg0DmgOut", "Hg2DmgOut", "MeHgDmgOut"]].sum(axis=1)
    hg_yldm['Particle'] = hg_yldm[["Hg0PmgOut", "Hg2PmgOut", "MeHgPmgOut"]].sum(axis=1)
    ...
    print("Dissolved Hg, {:.2f}g/year".format(hg_yldm['Dissolved'].sum()))
    print("Particle Hg, {:.2f}g/year".format(hg_yldm['Particle'].sum()))
...
def plot_():
    ...
    hg.Viz().barToOne(
        ...
        [0, 10],
        dates=hg_wt_dates,
        xlabel="Observed Hg in Stream $(ppb)$",
        ylabel="Simulated Hg in Stream $(ppb)$",
        output="hgInStr.jpg",
    )
    # sed
    hg.Viz().barToOne(
        ...
        [0, 120],
        width=5,
        xlabel="Observed Hg in Sediment $(ppm)$",
        ylabel="Simulated Hg in Sediment $(ppm)$",
        output="hgInSed.jpg",
    )
    # groundwater
    hg.Viz().gwlv(
...
def gw_hyd_sep():
    ...
    axes[0, 1].plot(gw02.index, gw02.sim_wt, c="orange", alpha=0.5)
    axes[0, 1].scatter(
        gw02.index,
        gw02.obd_wt,
        s=60,
    ...
    axes[1, 0].plot(gw03.index, gw03.sim_wt, c="g", alpha=0.5)
    axes[1, 0].scatter(
        gw03.index,
        gw03.obd_wt,
        s=60,
    ...
    axes[1, 1].plot(gw04.index, gw04.sim_wt, c="r", alpha=0.5)
    axes[1, 1].scatter(
        gw04.index, gw04.obd_wt, s=60, lw=1.5, alpha=0.5, zorder=10, marker="o", c="r"
    )
...
def temp():
    ...
    hg_yldm = hg_sub_dff.resample("ME").sum()
    hg_yldm = hg_yldm.groupby(hg_yldm.index.month).mean()
    print("\n" + "- " * 10 + "Hg testing" + "- " * 10)
    ...
    dff['ds']=hg_yldm[ds_cols].sum(axis=1)
    dff['pt']=hg_yldm[pt_cols].sum(axis=1)

    print(dff['ds'].sum())
    print(dff['pt'].sum())   
    # print("- " * 10)
    # print("{:.2f}g/year".format(hg_yldm))
...
if __name__ == "__main__":
    ...
    # wd = "/Users/seonggyu.park/Documents/windows_folder/spark_gits/gumu_project/gumu_pp_glm"
    wd = "/Users/seonggyu.park/Documents/windows_folder/gumu_org"
    os.chdir(wd)
    # temp()
    mod1 = hg.Hg(wd)
    sim_start = "1/1/2010"
